...
_FAKE_PROFILE = {
    ...
    "market_cap": 1.2e9,
    "current_price": 26.4,
    "summary": "AAOI designs optical networking products.",
    "revenue": 2.5e8,
    "profit_margin": -0.05,
}
...
def test_build_detail_degrades_without_recorded_inputs(tmp_path: Path) -> None:
    ...
    _seed_form4(db)
    detail = build_ticker_detail(
        _write_ledger(tmp_path, with_inputs=False),
        _CONFIG,
        "AAOI",
        db_path=db,
        today=_TODAY,
...
def test_detail_not_in_ledger(tmp_path: Path) -> None:
    ...
    detail = build_ticker_detail(
        ...
        db_path=db,
        today=_TODAY,
        profile_fetch=lambda t: {"sector": None, "industry": None},
    )
